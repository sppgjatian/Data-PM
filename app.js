...
async function handleUpload(e) {
    e.preventDefault();
    if (!currentFile) { showToast('Pilih file dulu!', 'error'); return; }

    const schoolName = document.getElementById('schoolName').value.trim();
    if (!schoolName) { showToast('Nama sekolah wajib diisi!', 'error'); return; }
    ...
    try {
        ...
        for (let i = 0; i < rows.length; i += 100) {
            const { error } = await supabase.from('recipients').insert(rows.slice(i, i + 100));
...
function updateStats() {
    const total = schoolData.reduce((sum, s) => sum + (s.row_count || 0), 0);
...
function renderSchools(data) {
    ...
    data.forEach(school => {
        ...
        card.onclick = () => openSchoolDetail(school);
        card.innerHTML = `
...
function filterSchools() {
    ...
    const filtered = schoolData.filter(s => s.school_name.toLowerCase().includes(search));
